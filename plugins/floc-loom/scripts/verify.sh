#!/bin/sh
# Repository-local verification for FLOC*Loom's custom-agent companion installer.
...
hash_agents() {
  if command -v shasum >/dev/null 2>&1; then
    shasum -a 256 "$1/floc-loom-luna-implementer.toml" "$1/floc-loom-terra-implementer.toml" "$1/floc-loom-sol-reviewer.toml" | shasum -a 256 | awk '{print $1}'
  elif command -v sha256sum >/dev/null 2>&1; then
    sha256sum "$1/floc-loom-luna-implementer.toml" "$1/floc-loom-terra-implementer.toml" "$1/floc-loom-sol-reviewer.toml" | sha256sum | awk '{print $1}'
  else
...
tmp_dir=$(mktemp -d "$tmp_base/floc-loom-verify.XXXXXX") || fail "could not create disposable verification directory"
case "$tmp_dir" in
  "$tmp_base"/floc-loom-verify.*) ;;
...
test -f "$setup" || fail "unified setup missing: $setup"
test -f "$runtime_inspector" || fail "runtime inspector missing: $runtime_inspector"
test -f "$ledger" || fail "execution ledger missing: $ledger"
test -f "$ledger_tests" || fail "execution ledger tests missing: $ledger_tests"
...
test -f "$execution_graphs" || fail "execution graph reference missing: $execution_graphs"

jq empty "$manifest"
pass "plugin manifest JSON is valid"
jq -e '.version == "0.4.0"' "$manifest" >/dev/null || fail "plugin manifest version is not 0.4.0"
pass "plugin manifest release version"
...
home_unset_target=$tmp_dir/home-unset-target
env -u HOME -u CODEX_HOME sh "$installer" --target-dir "$home_unset_target"
for agent_file in floc-loom-luna-implementer.toml floc-loom-terra-implementer.toml floc-loom-sol-reviewer.toml; do
  cmp -s "$templates/$agent_file" "$home_unset_target/$agent_file" || fail "explicit target failed when HOME and CODEX_HOME were unset: $agent_file"
done
env -u HOME -u CODEX_HOME sh "$installer" --help >/dev/null
pass "installer explicit target and help without HOME"

if sh "$installer" --target-dir /tmp/.. >/dev/null 2>&1; then
  fail "installer accepted a target path that resolves to the filesystem root"
fi
pass "installer canonical root refusal"
...
printf '%s\n' 'intentionally conflicting custom-agent template' > "$conflict_target/floc-loom-luna-implementer.toml"
if sh "$installer" --target-dir "$conflict_target"; then
  fail "installer accepted a differing destination file"
fi
test ! -e "$conflict_target/floc-loom-terra-implementer.toml" || fail "conflict refusal partially installed the Terra template"
test ! -e "$conflict_target/floc-loom-sol-reviewer.toml" || fail "conflict refusal partially installed the Sol template"
...
printf '%s\n' \
  ...
  '  jq -n '\''{marketplaceName:"floc-studio",installedRoot:"/fixture",alreadyAdded:false}'\''' \
  'elif [ "$1 $2" = "plugin add" ]; then' \
  '  jq -n --arg path "$FAKE_PLUGIN_DIR" '\''{pluginId:"floc-loom@floc-studio",name:"floc-loom",marketplaceName:"floc-studio",version:"0.4.0",installedPath:$path,authPolicy:"ON_INSTALL"}'\''' \
  'elif [ "$1 $2" = "plugin list" ]; then' \
  '  jq -n --arg path "$FAKE_PLUGIN_DIR" '\''{installed:[{pluginId:"floc-loom@floc-studio",name:"floc-loom",marketplaceName:"floc-studio",version:"0.4.0",installed:true,enabled:true,source:{source:"local",path:$path}}],available:[]}'\''' \
  'else' \
...
grep -Fq 'plugin list --json' "$fake_codex_log" \
  || fail "unified setup check did not inspect installed plugins"
pass "unified local setup and non-mutating check"
...
printf '%s\n' \
  ...
  "{\"type\":\"session_meta\",\"payload\":{\"id\":\"$runtime_success_id\",\"parent_thread_id\":\"00000000-0000-7000-8000-000000000000\",\"agent_role\":\"floc_loom_luna_implementer\",\"agent_path\":\"/root/fixture\",\"model_provider\":\"openai\",\"cwd\":\"/fixture/cwd\",\"base_instructions\":\"DO_NOT_LEAK_INSTRUCTIONS\"}}" \
...
strict_runtime_output=$(sh "$runtime_inspector" \
  ...
  "$runtime_success_id")
if ! printf '%s\n' "$strict_runtime_output" | jq -e '.agent_role == "floc_loom_luna_implementer" and .sandbox_policy_type == "danger-full-access"' >/dev/null; then
  fail "runtime inspector strict expected-value validation failed"
fi
if sh "$runtime_inspector" --sessions-dir "$runtime_sessions" --expected-role floc_loom_terra_implementer "$runtime_success_id" >/dev/null 2>&1; then
  fail "runtime inspector accepted an unexpected role"
fi
pass "runtime inspector strict expected-value validation"
...
printf '%s\n' "{\"type\":\"session_meta\",\"payload\":{\"id\":\"$runtime_missing_turn_id\",\"agent_role\":\"floc_loom_luna_implementer\"}}" > "$runtime_day/rollout-2026-08-01T00-00-01-$runtime_missing_turn_id.jsonl"
...
printf '%s\n' "{\"type\":\"session_meta\",\"payload\":{\"id\":\"$runtime_missing_model_id\",\"agent_role\":\"floc_loom_luna_implementer\"}}" '{"type":"turn_context","payload":{"effort":"max"}}' > "$runtime_day/rollout-2026-08-01T00-00-04-$runtime_missing_model_id.jsonl"
runtime_missing_effort_id=77777777-7777-7777-8777-777777777777
printf '%s\n' "{\"type\":\"session_meta\",\"payload\":{\"id\":\"$runtime_missing_effort_id\",\"agent_role\":\"floc_loom_luna_implementer\"}}" '{"type":"turn_context","payload":{"model":"gpt-5.6-luna"}}' > "$runtime_day/rollout-2026-08-01T00-00-05-$runtime_missing_effort_id.jsonl"
...
runtime_missing_sandbox_id=99999999-9999-7999-8999-999999999999
printf '%s\n' \
  "{\"type\":\"session_meta\",\"payload\":{\"id\":\"$runtime_missing_sandbox_id\",\"agent_role\":\"floc_loom_sol_reviewer\"}}" \
  '{"type":"turn_context","payload":{"model":"gpt-5.6-sol","effort":"high","cwd":"/fixture/cwd"}}' \
  > "$runtime_day/rollout-2026-08-01T00-00-07-$runtime_missing_sandbox_id.jsonl"
if sh "$runtime_inspector" \
  --sessions-dir "$runtime_sessions" \
  ...
  --require-permission-profile \
  "$runtime_missing_sandbox_id" >/dev/null 2>&1; then
  fail "runtime inspector accepted missing reviewer isolation metadata"
fi
pass "runtime inspector reviewer isolation metadata refusal"
...
for document in "$skill" "$contracts"; do
  grep -Fq 'agent_type: floc_loom_luna_implementer' "$document" || fail "missing Luna custom agent reference: $document"
  grep -Fq 'agent_type: floc_loom_terra_implementer' "$document" || fail "missing Terra custom agent reference: $document"
  grep -Fq 'agent_type: floc_loom_sol_reviewer' "$document" || fail "missing Sol custom agent reference: $document"
...
grep -Fq 'offer to run the same' "$skill" || fail "skill does not provide approval-gated first-use agent setup"
grep -Fq '../../scripts/inspect-agent-runtime.sh' "$skill" || fail "skill does not resolve the runtime inspector relative to SKILL.md"
grep -Fq '../../scripts/ledger.py' "$skill" || fail "skill does not resolve the execution ledger relative to SKILL.md"
grep -Fq 'references/execution-graphs.md' "$skill" || fail "skill does not route graph-shaped work to its reference"
grep -Fq 'execution-graphs.md' "$contracts" || fail "role contracts do not apply graph and domain routing"
grep -Fq 'Luna Max is the preferred implementation lane for frontend and backend nodes' "$execution_graphs" || fail "execution graph reference does not make Luna Max the bounded frontend/backend default"
grep -Fq 'Use Terra before implementation' "$execution_graphs" || fail "execution graph reference lacks Terra escalation criteria"
grep -Fq '## Design authority' "$execution_graphs" || fail "execution graph reference does not preserve studio design authority"
grep -Fq '## Frontend profile' "$execution_graphs" || fail "execution graph reference lacks frontend verification"
grep -Fq '## Backend profile' "$execution_graphs" || fail "execution graph reference lacks backend verification"
grep -Fq '## Full-stack profile' "$execution_graphs" || fail "execution graph reference lacks full-stack integration"
grep -Fqi 'public native spawn/details metadata first' "$skill" || fail "skill does not require public runtime metadata first"
grep -Fqi 'studio-approved design sources as immutable implementation contracts' "$skill" || fail "skill does not block model-owned design decisions"
...
python3 "$ledger_tests"
pass "execution ledger acceptance, scope, mutation, and behavioral-isolation tests"
...
printf '%s\n' "VERIFY PASSED: FLOC*Loom companion-agent checks completed in $tmp_dir"
